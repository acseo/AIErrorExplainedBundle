...
/**
 * This class is almost a copy of Symfony\Component\ErrorHandler\ErrorHandler
 * Only the private function renderException() has been overriden
 * in order to use HtmlErrorRendererWithAISuggestion instead of HtmlErrorRenderer
 */
class ErrorHandler
{
    ...
    {
        $renderer = \in_array(\PHP_SAPI, ['cli', 'phpdbg'], true) ? new CliErrorRenderer() : new HtmlErrorRendererWithAISuggestion($this->debug);

        $rc = new \ReflectionClass(get_class($renderer));
        $methods = ['renderOverride', 'render'];
        foreach($methods as $method)
        {
            if ($rc->hasMethod($method)) {
                $exception = $renderer->$method($exception);
                break;
            }
        }
